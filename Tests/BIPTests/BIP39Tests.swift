...
extension BIP39Tests {
    ...
    func testVector0(_ vector: Vector0) throws {
        let seed = vector.mnemonica.seed(with: .ethereum(
            password: vector.password,
            iterations: vector.iterations,
            klength: vector.keyLength
        ))

        #expect(seed.hexadecimalString == vector.seed)

        let mnemonica1 = try Mnemonica(
            entropy: vector.mnemonica.entropy,
            glossary: vector.mnemonica.glossary
        )

        #expect(mnemonica1.rawValue == vector.mnemonica.rawValue)
    }
}
...
extension BIP39Tests {
    ...
    func testVector1(_ vector: Vector1) throws {
        let mnemonica = vector.mnemonica
        let entropy = mnemonica.entropy
        let seed = vector.seed

        #expect(mnemonica.seed(with: .ethereum(password: "TREZOR")).hexadecimalString == seed)
        let mnemonica1 = try Mnemonica(entropy: entropy, glossary: mnemonica.glossary)
        #expect(mnemonica1.rawValue == mnemonica.rawValue)
    }
}
...
extension BIP39Tests {
    ...
    @Test("Vector TON", arguments: [
        (
            "spoon keep labor chalk cover grid before struggle physical cram empower trigger outdoor skill clutch loud venture assume inch put idle crop mask mesh",
            "414f2252026abb2dc5e5f2b6a54038d46ccac62fef466e91a19c16b04e3225dc"
        ),
    ])
    func testVectorTON(_ vector: VectorTON) throws {
        let seed = vector.mnemonica.seed(with: .ton())
        #expect(seed.hexadecimalString == vector.seed)
    }
}
